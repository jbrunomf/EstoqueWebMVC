...
namespace EstoqueWeb.Controllers
{
    public class ProdutoController : Controller
    {
        ...
        public async Task<IActionResult> Index(string categoriaProduto, string textoBusca)
        {
            IQueryable<CategoriaModel> categoriaQuery = from p in _context.Produtos orderby p.Categoria select p.Categoria;

            var produtos = from p in _context.Produtos select p;
            if (!String.IsNullOrEmpty(textoBusca))
            {
                // produtos = produtos.Where(p => p.Nome!.ToLower().Contains(textoBusca.ToLower()));
                produtos = _context.Produtos
                    .Where(p => p.Nome.Contains(textoBusca)
                                || p.Codigo.Contains(textoBusca));
            }

            if (!String.IsNullOrEmpty(categoriaProduto))
            {
                produtos = produtos.Where(p => p.Categoria.Nome == categoriaProduto);
            }

            var produtoCategoriaVm = new ProdutoViewModel
            {
